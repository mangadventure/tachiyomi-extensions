...
abstract class MangAdventure(
    ...
    open val categories = DEFAULT_CATEGORIES

    /** The site's manga status names. */
    open val statuses = arrayOf("Any", "Completed", "Ongoing")

    /** The site's sort order labels that correspond to [SortOrder.values]. */
    open val orders = arrayOf("Title", "Latest upload", "Chapter count")
    ...
    private val apiUrl by lazy { "$baseUrl/api/v$versionId" }
    ...
    override fun latestUpdatesRequest(page: Int) =
        Uri.parse("$apiUrl/series").buildUpon().run {
    ...
    override fun searchMangaRequest(
        ...
        if (query.startsWith(SLUG_QUERY)) {
            appendQueryParameter("slug", query.drop(SLUG_QUERY.length))
        } else {
            appendQueryParameter("page", page.toString())
    ...
    override fun chapterListRequest(manga: SManga) =
        Uri.parse("$apiUrl/chapters").buildUpon().run {
    ...
    override fun pageListRequest(chapter: SChapter) =
        Uri.parse("apiUrl/pages").buildUpon().run {
    ...
    override fun mangaDetailsParse(response: Response) =
        response.decode<Series>().let { series ->
            SManga.create().apply {
                url = series.url
                title = series.title
                thumbnail_url = series.cover
    ...
    override fun fetchMangaDetails(manga: SManga): Observable<SManga> =
        client.newCall(GET("$apiUrl/series/${manga.slug}", headers))
    ...
    /** The path components (series, volume, number) of the chapter. */
